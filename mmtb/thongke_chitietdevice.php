       ...
       $devicetype = isset($_GET['devicetype']) ? $_GET['devicetype'] : '' ;
        $deptdevice = isset($_GET['deptdevice']) ? $_GET['deptdevice'] : '' ;
            ...
            if(empty($deptdevice))
            {
             ...
             {
                $query_total = "SELECT *, DATE_FORMAT(device.device_update_date, '%d-%m-%Y') as devive_update_date  from device join phongban on device.dept=phongban.dept_id WHERE device.device_type like '%$devicetype%' ORDER BY device_type_id ASC" ; 
             }
            }            
            else
            {    
                ...
                if(empty($devicetype))
                {
                $query_total = "SELECT *, DATE_FORMAT(device.device_update_date, '%d-%m-%Y') as devive_update_date from device join phongban on device.dept=phongban.dept_id WHERE phongban.dept_id = '$deptdevice' ORDER BY device_type_id ASC" ; 
             }
                else
             {
                $query_total = "SELECT *, DATE_FORMAT(device.device_update_date, '%d-%m-%Y') as device_update_date  from device  join phongban on device.dept=phongban.dept_id WHERE dept_id = '$deptdevice' AND device.device_type like '%$devicetype%' ORDER BY device_type_id ASC" ; 
             }               
    ...
    <!-- <div class="col-lg-12"> -->
        ...
        <div class="col-lg-12">
                <div class="row_list_device">
                        ...
                        <table id='empTable' class="display custom-tt" style="width:99.5%;">
  <thead>
    <tr>
      ...
      <th rowspan="1" colspan="9"scope="col" style="width:500px; text-align: center;"> Cấu hình thiết bị</th>
      <th  rowspan="2" colspan="1" scope="col" style="width:50px; text-align: center;">Số lần bơm mực</th>
      <th  rowspan="2" colspan="1" scope="col" style="width:50px; text-align: center;">Phòng ban</th>
      <th  rowspan="2" colspan="1" scope="col" style="width:70px; text-align: center;">Nhân sự sử dụng</th>
      <th  rowspan="2" colspan="1" scope="col" style="width:150px; text-align: center;">Note</th>      
      <th  rowspan="2" colspan="1" scope="col" style="width:70px; text-align: center;">Ngày cập nhật</th>
      <?php
      if ($_SESSION['admin_mmtb'] >= 1 || $_SESSION['admin'] >=3) {      
      ?>
  ...
  ?>
   <?php
    ...
    <tr> 
      ...
      if ( $_SESSION['admin_mmtb'] >= 1 || $_SESSION['admin'] >=3) {      
      ?>
      <td style=" text-align: center ;" ><a href="updatedevice.php?deviceid=<?php echo $row['device_name']; ?>&func=update-device"><font size="5" color="green"><i class="fa fa-pencil-square"></i> </font></a><a href="updatedevice.php?deviceid=<?php echo $row['device_name']; ?>&func=remove-device"><font size="5" color="red"><i class="fa fa-trash-o"> </i> </font></a></td>  
      <?php
  }
  ?> 
    </tr> 
...
<?php
      ...
      ?>
<div class="col-lg-1"> <a href="updatedevice.php" class="btn btn-primary">Thêm</a></div>
<?php
  }
  ?>  
                </div><!--/.row-->
        ...
        $(document).ready(function(){
            ...
            var empDataTable = $('#empTable').DataTable({                   
                ...
                buttons: [
                    {
                        extend: 'copyHtml5',
                        title: 'Danh sách thiết bị đơn vị ' + donvi ,
                    },
                    {
                        extend: 'pdfHtml5',
                        title: 'Danh sách thiết bị đơn vị ' + donvi ,
